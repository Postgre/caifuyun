 ...
 *
 */
public class IndividualCustomersActivity extends BaseActivity implements OnClickListener,Callback{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_individual_customers);
		initView();
		initData();
	}
	private void initData(){
		Intent intent=getIntent();
		if(intent.hasExtra("userId")){//个人客户
			setHeaderFields(0, R.string.individual_customers, 
					0,R.drawable.ic_action_back, 0,R.drawable.ic_action_more);
			currentTag=INDIVIDUAL;
			normalId=intent.getIntExtra("userId", -1);
		...
		if(intent.hasExtra("organizationId")){//机构客户
			setHeaderFields(0, R.string.organization_customers, 
					0,R.drawable.ic_action_back, 0,R.drawable.ic_action_more);
			currentTag=ORGANIZATION;
			normalId=intent.getIntExtra("organizationId", -1);
	...
	private void initView() {
		headerView=getLayoutInflater().inflate(R.layout.include_header_custom_list,null);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			if (IndividualCustomersActivity.this.getCurrentFocus() != null) {
				if (IndividualCustomersActivity.this.getCurrentFocus().getWindowToken() != null) {
					//调用系统自带的隐藏软键盘
					((InputMethodManager)getSystemService(INPUT_METHOD_SERVICE)).
					hideSoftInputFromWindow(IndividualCustomersActivity.this.getCurrentFocus().
		...
		try {
			if(response.getInt("status")==0){
				...
				if(user!=null){
					...
					if(com.zepan.caifuyun.util.StringUtil.isNullOrHttp(user.getAvatar())){
						((ImageView)headerView.findViewById(R.id.iv_head)).setImageURI(Uri.parse(user.getAvatar()));
					}else{
						((ImageView)headerView.findViewById(R.id.iv_head)).setImageResource(R.drawable.ic_default_head);
					}

					((TextView)headerView.findViewById(R.id.tv_name)).setText(user.getUserName());//用户名或者公司名
					((RatingBar)headerView.findViewById(R.id.ratingBar1)).setRating((user.getMaturity()));//成熟度
					((RatingBar)headerView.findViewById(R.id.ratingBar2)).setRating(user.getPower());//实力
					((RatingBar)headerView.findViewById(R.id.ratingBar3)).setRating(user.getRiskAppetite());//风险
					((TextView)headerView.findViewById(R.id.tv_emai)).setText(user.getEmail());//邮箱
					//((TextView)findViewById(R.id.tv_phone)).setText(user.getRelationUser());

					if(jsonAccount.has("dbcVarchar1")){//主要联系人
						((TextView)headerView.findViewById(R.id.tv_mian_user)).setText(user.getRelationUser());
					}
					if(!"".equals(user.getTags())&&user.getTags()!=null){
						...
						for(int i=0;i<user.getTags().size();i++){
							...
							tagsList.add(user.getTags().get(i));
						}
						((TextView)headerView.findViewById(R.id.tv_tags)).setText(tag);
					}
	...
	public void onClick(View v) {
		...
		case R.id.iv_right:
			...
			infoDailog.setOnItemClick(new OnItemClick() {
				...
				public void itemClick(AdapterView<?> parent, View view, int position, long id) {
					...
					case 1:
						infoDailog.dismiss();
						Intent intent=new Intent(IndividualCustomersActivity.this, NewIndividualCustomersActivity.class);
						if(currentTag==INDIVIDUAL){
							intent.putExtra("updateIndividual","updateIndividual");
							intent.putExtra("updateIndividualId", normalId);

						}else if(currentTag==ORGANIZATION){
							intent.putExtra("updateOrganization","updateOrganization");
							intent.putExtra("updateOrganizationId", normalId);
						}
		...
		case R.id.iv_email://邮件
			...
			//到时候该成动态的
			intentEmail.setData(Uri.parse("mailto:"+((TextView)findViewById(R.id.tv_emai)).getText().toString()));  
	...
	private void getRecodeList(){
		try {
			...
			String uid=getStringLocalData("uid");
			String tid=getStringLocalData("tid");
			String url=Url.RecodeGetAll+"?token="+token+"&accountId="+normalId+"&uid="+uid+"&tid="+tid;
			request(url, null, Method.GET, new IRequestCallBack() {
	...
	private void getUpdateLevel(){
		try {
			...
			String uid =getStringLocalData("uid");
			String tid =getStringLocalData("tid");
			String url=Url.UpdateLevel+"?token="+token+"&accountId="+normalId+"&level="+2+"&uid="+uid+"&tid="+tid;
			request(url, null, Method.GET, new IRequestCallBack() {
	...
	private void getDelete(){
		try {
			...
			String uid=getStringLocalData("uid");
			String tid=getStringLocalData("tid");
			String url=Url.Delete+"?token="+token+"&accountIds="+normalId+"&uid="+uid+"&tid="+tid;
			request(url, null, Method.GET, new IRequestCallBack() {
