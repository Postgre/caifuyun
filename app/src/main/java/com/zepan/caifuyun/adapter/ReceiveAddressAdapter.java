...
public class ReceiveAddressAdapter extends BaseAdapter{
	...
	public View getView(int position,View convertView, ViewGroup parent) {
		...
		if(convertView==null){
			...
			convertView=inflater.inflate(R.layout.listitem_forcontract, null);
			...
			holder.iv_choice_right=(ImageView)convertView.findViewById(R.id.iv_choice_right);
		...
		if(list.get(position)!=null){
			...
			if(list.get(position).getDefault_address()==1){
				holder.iv_choice_right.setVisibility(View.VISIBLE);
			}else{
				holder.iv_choice_right.setVisibility(View.GONE);
			}
	...
	class HolderView{
		...
		TextView address;//地址
		TextView zip;//邮编
		ImageView iv_choice_right;//选中的图标
	}
}
